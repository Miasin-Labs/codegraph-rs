...
use codegraph_analysis::context::ContextOptions;
...
fn main() {
    ...
    let raw = session
        ...
        .expect("query_raw");
    let env = codegraph_analysis::schema::wrap_query_result(raw);
    ...
    let schema_text = codegraph_analysis::schema::json_schema_for(
        codegraph_analysis::schema::PayloadKind::QueryResult,
    );
    println!("{schema_text}");
    ...
    let loaded = codegraph_analysis::overlay::load_base_snapshot(&snap_path).expect("load");
    println!(
    ...
    let dir = codegraph_analysis::data_dir::resolve_data_dir(&root);
    println!("resolved = {}", dir.display());

    divider("worktree — explicit check (cwd vs workspace root)");
    let cwd = std::env::current_dir().unwrap();
    match codegraph_analysis::worktree::detect_worktree_index_mismatch(&cwd, &root) {
