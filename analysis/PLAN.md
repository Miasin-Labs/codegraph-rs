...
are missing from `codegraph-analysis`. This bridges the gap while preserving codegraph-analysis's
...
**What codegraph-analysis has:** Nothing — no per-node metrics.

### Implementation

Create `analysis/src/complexity.rs`:
...
**What codegraph-analysis has:** Nothing — traversal is at the call-graph level only.

### Implementation

Create `analysis/src/cfg.rs`:
...
**What codegraph-analysis has:** `taint` operator traces a named parameter through call
chains, but doesn't extract structured dataflow per-function.

### Implementation

Create `analysis/src/dataflow.rs`:
...
**What codegraph-analysis has:** Nothing (strata gives call-depth layers but not clusters).

### Implementation

Create `analysis/src/communities.rs`:
...
**What codegraph-analysis has:** 4 (Rust, TypeScript, Python, Go).
...
**What codegraph-analysis has:** `history.rs` tracks `last_modified_revision` per node
and the `since N` DSL filter, but no co-change correlation.

### Implementation

Create `analysis/src/co_change.rs`:
...
**What codegraph-analysis has:** Substring matching only (`codegraph_search` does
