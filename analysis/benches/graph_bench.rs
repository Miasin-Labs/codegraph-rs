//! Criterion benchmarks for `codegraph-analysis` (Phase 16).
//!
//! Run with: `cargo bench -p codegraph-analysis`
//!
//! Benchmarks:
...
use codegraph_analysis::csr::CsrSnapshot;
use codegraph_analysis::dsl::{QueryConfig, run_query_expr};
use codegraph_analysis::edges::{EdgeData, EdgeKind};
use codegraph_analysis::graph::CodeGraph;
use codegraph_analysis::nodes::{NodeData, NodeId, NodeKind, Span, Visibility};
use codegraph_analysis::traversal::{
    TraversalConfig,
    TraversalDirection,
    traverse_csr,
    traverse_petgraph,
...
fn bench_tarjan_articulation(c: &mut Criterion) {
    ...
    c.bench_function("tarjan_articulation_2k_chain", |b| {
        b.iter(|| black_box(codegraph_analysis::analysis::critical_nodes(&g)));
    });
}
...
fn bench_query_cache_hit(c: &mut Criterion) {
    use codegraph_analysis::incremental::{QueryCache, QueryKey, ReadSet};
...
fn bench_query_cache_miss(c: &mut Criterion) {
    use codegraph_analysis::incremental::{QueryCache, QueryKey};
    let cache: QueryCache<Vec<u32>> = QueryCache::new();
