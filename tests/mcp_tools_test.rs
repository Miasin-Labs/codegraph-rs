...
fn arch_overview_lists_in_scope_modules_and_symbols_only() {
    ...
    // src/core is the subsystem under test; src/app.ts is OUTSIDE it and
    // depends on it (so it must appear as an external dependent, never as a
    // listed module).
    write(
    ...
    // app.ts is out of scope: it must NOT be a listed module (header form
    // "src/app.ts [lang] — N symbols"), though it may appear as a dependent.
    assert!(
...
fn xref_lists_incoming_references_to_a_symbol() {
    ...
    // caller() calls target(), so it must appear as an incoming reference.
...
fn paths_finds_call_chain_from_source_to_sink() {
    ...
    // source -> mid -> sink, so the chain must pass through mid.
...
fn verify_roles_proposes_then_proves_emitting_the_deviant_caller() {
    let _env = env_read();
    let dir = TempDir::new().unwrap();
    // `delete_order` is reached by five handlers; four call `check_auth` first,
    // `handler_e` does not — the lone deviation the graph corroborates.
    // The guard call must sit on an earlier line than the sink call (the
    // precedence check is line-ordered), so each handler spans several lines.
    write(
    ...
    // The deviant caller is flagged, tagged llm, via the missing-guard template.
    assert!(
    ...
    // The compliant handlers must NOT be flagged.
    assert!(
...
fn verify_roles_drops_hallucinated_sink_without_enough_callers() {
    ...
    let text = res.text();
    // A sink with no callers fails verification → zero findings escape.
    assert!(
