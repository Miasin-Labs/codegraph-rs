mod path_traversal_prevention {
    ...
    async fn setup(root: &Path) -> CodeGraph {
        write(
    ...
    async fn reads_code_for_valid_nodes_within_project() {
        let dir = TempDir::new().unwrap();
        let cg = setup(dir.path()).await;
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn returns_none_for_non_existent_node() {
        let dir = TempDir::new().unwrap();
        let cg = setup(dir.path()).await;
