...
fn target() -> &'static dyn AgentTarget {
    get_target("prime").unwrap()
}

fn global_file(env: &TestEnv) -> PathBuf {
    env.home().join(".prime/agent/settings.json")
}

fn local_file(env: &TestEnv) -> PathBuf {
    env.cwd().join(".prime/agent/settings.json")
}

#[test]
fn prime_global_install_writes_mcp_servers_shape() {
    let env = TestEnv::new();
...
#[test]
fn prime_local_install_targets_project_dir() {
    let env = TestEnv::new();
...
#[test]
fn prime_honors_env_dir_override_for_global() {
    let env = TestEnv::new();
    let relocated = env.home().join("relocated-prime");
    env::set_var("PRIME_AGENT_CODING_AGENT_DIR", &relocated);
...
#[test]
fn prime_preserves_sibling_settings_and_servers() {
    let env = TestEnv::new();
...
#[test]
fn prime_reinstall_is_unchanged() {
...
#[test]
fn prime_uninstall_drops_entry_keeps_siblings_and_file() {
    let env = TestEnv::new();
    let file = global_file(&env);
    write(
        &file,
...
#[test]
fn prime_uninstall_handles_absence() {
    let _env = TestEnv::new();
    assert_eq!(
        target().uninstall(Location::Global).files[0].action,
        FileAction::NotFound
...
#[test]
fn prime_detection_and_print_config_match_source() {
    let env = TestEnv::new();
    assert!(!target().detect(Location::Global).installed);
    fs::create_dir_all(env.home().join(".prime/agent")).unwrap();
    assert!(target().detect(Location::Global).installed);
