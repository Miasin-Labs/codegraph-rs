...
| 3 | Session memory: fixed `history` ingest + redaction, then `recall` and a ≤1 KB session digest | 45% cross-session re-discovery; ~57M tok | ingest fixed (one row per call, idempotent, 3.8× over-count gone, redaction of sudo passwords/keys/JWTs/emails); then shipped: cross-session memory (Claude Code, opencode, JFC), `codegraph history recall`, opt-in `codegraph_recall`, and a ≤1 KB first-prompt digest — on this repo 45.9% of re-read files and 19% of re-searched identifiers are covered by recall |
| 4 | Explore that ranks and folds in 1-hop neighbours (edges, co-change, tests) under a hard budget | 60% of replay misses; 0.49 single-call ceiling | shipped: ranked `relatedFiles` rows — replay recall 0.34 → 0.52 (brief), 0.30 → 0.47 (natural language), same size and latency; prose words no longer pin seeds |
...
| 6 | Text search over indexed files returning hits + enclosing symbol, per-file caps | 27% of greps are text; >100-hit searches = half of search tokens | shipped: `codegraph_grep` (12th default tool) — 25 real greps: 273 KB via rg → 56 KB, broad searches ~6× smaller, same latency |
