...
//!
//! Port of `src/mcp/server-instructions.ts`. The string below is the single
//! source of truth for agent-facing tool guidance (issue #529) and must stay
//! byte-identical to the TS export.
...
mod tests {
    ...
    #[test]
    fn instructions_match_the_ts_source_shape() {
        // Anchors that pin the byte-identical port (full-text equality against
        // the TS file is asserted in tests/mcp_server_test.rs, where the repo
        // checkout is available).
        assert!(
            SERVER_INSTRUCTIONS
                .starts_with("# Codegraph — code intelligence over an indexed knowledge graph\n")
        );
        assert!(SERVER_INSTRUCTIONS.ends_with("structural context they don't have.\n"));
