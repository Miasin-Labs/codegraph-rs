...
## Intentional MCP divergence: official rmcp runtime (2026-08-18)

The Rust MCP server no longer attempts byte-for-byte parity with the TS
session/transport implementation. It uses rmcp 3.1.3 for lifecycle,
negotiation, JSON-RPC dispatch, cancellation, progress, logging, roots, and
serialization. Consequences:

1. Rust negotiates rmcp's `2025-11-25` protocol instead of the TS ceiling of
   `2025-06-18`.
2. Direct, daemon, and daemon-failure fallback use the same
   `CodeGraphService`; the TS local-handshake semantic proxy has no Rust
   equivalent.
3. The Unix proxy retains only the TS-compatible daemon hello/version check,
   socket rendezvous, and PPID watchdog, then forwards bytes unchanged.
4. Tool annotations, dynamic `tools/list_changed`, progress, cooperative
   cancellation, logging, and roots refresh remain CodeGraph policies exposed
   through rmcp.
5. Deprecated initialize `rootUri` is ignored because it is absent from rmcp's
   typed model; clients use roots or `--path`.
6. Explore applies its adaptive limit to the complete structured payload, so
   relationship/additional-file metadata cannot bypass the source budget and
   trigger host truncation.

The daemon lockfile/hello bytes and `SERVER_INSTRUCTIONS` remain parity-pinned
where they are still shared contracts. See `notes/rmcp-gaps.md` for the full
migration record.
